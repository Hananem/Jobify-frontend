...
const API_URL = 'https://jobify-kefc.onrender.com/api/events';
...
const eventsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    .addCase(fetchAllEvents.rejected, (state, action) => {
      ...
      .addCase(countEvents.pending, (state) => {
        state.status = 'loading';
      })
      .addCase(countEvents.fulfilled, (state, action) => {
        state.status = 'succeeded';
        state.totalCount = action.payload; // Update the total count
      })
      .addCase(countEvents.rejected, (state, action) => {
        state.status = 'failed';
        state.error = action.payload || 'Failed to count events';
      });
  },
});
